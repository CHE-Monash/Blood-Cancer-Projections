...
#   run_table1_change_cri()       -> Table 1 change CrIs from output/apc_results.rds
#                                    (step 10 of run_apc_model; no refit)
...
# ----------------------
# Table 1 change CrIs (2021 -> 2045, draw-level)
# ----------------------
...
# Added round 3 [QL: CrI consistency across Tables 1-3]. Folded in from
# code/table_1_change_cri.R on 2026-09-16: called as step 10 of
# run_apc_model(), and callable on its own after source("code/apc_model.R")
# to rebuild the CSV from output/apc_results.rds without refitting.
...
run_apc_model <- function(B = 1000,
  ...
  # 10. Table 1 change CrIs from the stored draws (reads the RDS just saved;
  #     no refit, no RNG)
  run_table1_change_cri(save_dir = save_dir)

  if (verbose) {
    sz <- file.info(file.path(save_dir, "apc_results.rds"))$size / 1024^2
    cat(sprintf("Wrote %s (%.1f MB) and 6 CSVs + Figure 1\n",
